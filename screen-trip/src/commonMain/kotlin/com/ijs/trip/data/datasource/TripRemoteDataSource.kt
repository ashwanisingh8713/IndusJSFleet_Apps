...
import com.ijs.trip.TAG_TRIP_REMOTE_DS
...
class TripRemoteDataSourceImpl(
    ...
    override suspend fun getTrips(token: String): TripApiResponse<List<TripDto>> {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Fetching trips")
            val response: HttpResponse = httpClient.get(baseUrl) {
                header(HttpHeaders.Authorization, "Bearer $token")
            }
            parseListResponse(response)
        } catch (e: Exception) {
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to fetch trips: ${e.message}", e)
    ...
    override suspend fun getTripById(token: String, id: String): TripApiResponse<TripDto> {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Fetching trip: $id")
            val response: HttpResponse = httpClient.get("$baseUrl/$id") {
                header(HttpHeaders.Authorization, "Bearer $token")
            }
            parseSingleResponse(response)
        } catch (e: Exception) {
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to fetch trip: ${e.message}", e)
            TripApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
    ...
    override suspend fun createTrip(token: String, request: CreateTripRequest): TripApiResponse<TripDto> {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Creating trip at URL: $baseUrl")
            logger.d(TAG_TRIP_REMOTE_DS, "Request body: $request")
            val response: HttpResponse = httpClient.post(baseUrl) {
                header(HttpHeaders.Authorization, "Bearer $token")
                contentType(ContentType.Application.Json)
                setBody(request)
            }
            logger.d(TAG_TRIP_REMOTE_DS, "Response status: ${response.status}")
            val bodyText = response.bodyAsText()
            logger.d(TAG_TRIP_REMOTE_DS, "Response body: $bodyText")
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to create trip: ${e.message}", e)
            TripApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
    ...
    override suspend fun updateTrip(token: String, id: String, request: UpdateTripRequest): TripApiResponse<TripDto> {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Updating trip: $id")
            logger.d(TAG_TRIP_REMOTE_DS, "Update request - tripPrice: ${request.tripPrice}, weightUnit: ${request.weightUnit}, cargoWeight: ${request.cargoLoadingWeight}")
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to update trip: ${e.message}", e)
            TripApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
    ...
    override suspend fun updateTripStatus(token: String, id: String, status: String): TripApiResponse<TripDto> {
        return try {
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to update trip state: ${e.message}", e)
            TripApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
    ...
    override suspend fun updateTripProgress(
        ...
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Updating trip progress: $tripId")
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to update trip progress: ${e.message}", e)
            TripApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
    ...
    override suspend fun updateTripLocation(token: String, tripId: String, lat: Double, lng: Double): TripApiResponse<TripDto> {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Updating trip location: $tripId")
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to update trip location: ${e.message}", e)
            TripApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
    ...
    override suspend fun cancelTrip(token: String, id: String): TripApiResponse<Unit> {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Cancelling trip: $id")
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to cancel trip: ${e.message}", e)
            TripApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
    ...
    override suspend fun getTripsByVehicleId(token: String, vehicleId: String): TripApiResponse<List<TripDto>> {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Fetching trips for vehicle: $vehicleId")
            val response: HttpResponse = httpClient.get("${ApiConfig.BASE_URL}/vehicles/$vehicleId/trips") {
                header(HttpHeaders.Authorization, "Bearer $token")
            }
            parseListResponse(response)
        } catch (e: Exception) {
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to fetch trips for vehicle: ${e.message}", e)
            TripApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
    ...
    override suspend fun getTripStops(token: String, tripId: String): TripStopsApiResponse {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Fetching stops for trip: $tripId")
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to fetch trip stops: ${e.message}", e)
    ...
    override suspend fun createTripStop(token: String, tripId: String, request: CreateTripStopRequest): TripStopApiResponse {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Creating stop for trip: $tripId")
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to create trip stop: ${e.message}", e)
    ...
    override suspend fun updateTripStop(token: String, tripId: String, stopId: String, request: UpdateTripStopRequest): TripStopApiResponse {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Updating stop $stopId for trip: $tripId")
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to update trip stop: ${e.message}", e)
            TripStopApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
    ...
    override suspend fun markStopCompleted(token: String, tripId: String, stopId: String): TripStopApiResponse {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Marking stop $stopId as completed for trip: $tripId")
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to mark stop as completed: ${e.message}", e)
            TripStopApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
    ...
    override suspend fun deleteTripStop(token: String, tripId: String, stopId: String): TripApiResponse<Unit> {
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Deleting stop $stopId from trip: $tripId")
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to delete trip stop: ${e.message}", e)
            TripApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
    ...
    override suspend fun getTripStateHistory(
        ...
        return try {
            logger.d(TAG_TRIP_REMOTE_DS, "Fetching trip state history: $id, page=$page")
            ...
            val bodyText = response.bodyAsText()
            logger.d(TAG_TRIP_REMOTE_DS, "Trip state history response: $bodyText")
            if (response.status.isSuccess()) {
            ...
            logger.e(TAG_TRIP_REMOTE_DS, "Failed to fetch trip state history: ${e.message}", e)
            TripApiResponse(success = false, message = ApiErrorHandler.getNetworkErrorMessage(e))
        }
    }
}
