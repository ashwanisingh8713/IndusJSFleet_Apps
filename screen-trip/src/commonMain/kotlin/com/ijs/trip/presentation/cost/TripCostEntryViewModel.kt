...
import com.ijs.trip.TAG_TRIP_COST_VM
...
class TripCostEntryViewModel(
    ...
    private suspend fun syncDriverExpenses(trip: Trip, entries: List<CostEntryRow>) {
        ...
        if (driverIdInt == null || driverIdInt == 0) {
            logger.w(TAG_TRIP_COST_VM, "Cannot sync driver expenses: no driver assigned to trip ${trip.id}")
            return
        }

        if (tripIdInt == null) {
            logger.w(TAG_TRIP_COST_VM, "Cannot sync driver expenses: invalid trip ID ${trip.id}")
            return
        }
        ...
        if (driverExpenseEntries.isEmpty()) {
            logger.d(TAG_TRIP_COST_VM, "No driver expenses to sync for trip ${trip.id}")
            return
        }

        logger.d(TAG_TRIP_COST_VM, "Syncing ${driverExpenseEntries.size} driver expense(s) for driver $driverIdInt from trip ${trip.id}")
            ...
            try {
                ...
                if (driverCostRequest != null) {
                    when (val result = costsRepository.createDriverCost(
                        ...
                        is Result.Success -> {
                            successCount++
                            logger.d(TAG_TRIP_COST_VM, "Driver expense synced: ${entry.costTypeLabel} - ₹${entry.amount}")
                        }
                        is Result.Error -> {
                            failureCount++
                            // Log error but don't fail the trip cost save
                            logger.w(TAG_TRIP_COST_VM, "Failed to sync driver expense '${entry.costTypeLabel}': ${result.message}")
                        }
                        is Result.Loading -> { /* ignore */ }
                    }
                }
                ...
                failureCount++
                // Log error but don't fail the trip cost save
                logger.e(TAG_TRIP_COST_VM, "Error syncing driver expense '${entry.costTypeLabel}': ${e.message}", e)
            }
        }

        if (successCount > 0) {
            logger.i(TAG_TRIP_COST_VM, "Driver expense sync complete: $successCount synced, $failureCount failed")
        }
    }
}
