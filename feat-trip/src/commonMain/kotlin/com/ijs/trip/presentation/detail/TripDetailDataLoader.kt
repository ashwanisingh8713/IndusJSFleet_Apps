...
internal class TripDetailDataLoader(
    ...
    private val log = Logger.withTag("TripDetailDataLoader")
    ...
    suspend fun loadTrip(tripId: String) {
        ...
        withContext(dispatcherProvider.io) {
            ...
            log.d { "TripDetailDataLoader - userRole: '$userRole', normalized: '$normalizedRole'" }
    ...
    suspend fun loadTripPayments(tripId: String) {
        if (tripPaymentRepository == null) {
            log.d { "TripPaymentRepository not available, skipping payment load" }
            return
        }
        ...
        if (!state.canViewTripPrice) {
            log.d { "User role '${state.userRole}' cannot view payments, skipping" }
            return
        }
        ...
        withContext(dispatcherProvider.io) {
            when (val result = tripPaymentRepository.getTripPayments(tripId)) {
                is Result.Success -> {
                    ...
                    log.d { "Loaded ${payments.size} payments for trip $tripId" }
                }
                is Result.Error -> {
                    stateManager.updateTripState { copy(isLoadingPayments = false) }
                    log.e { "Failed to load trip payments: ${result.message}" }
                }
                is Result.Loading -> { /* Already handled */ }
            }
        }
        ...
        withContext(dispatcherProvider.io) {
            getVehiclesUseCase().collectLatest { result ->
                when (result) {
                    ...
                    is Result.Error -> log.e { "Failed to load vehicles: ${result.message}" }
                    is Result.Loading -> { /* Already handled */ }
                }
            }
        }
