...
internal class TripDetailActionHandler(
    ...
    private val log = Logger.withTag("TripDetailActionHandler")
    ...
    suspend fun saveChanges() {
        ...
        withContext(dispatcherProvider.io) {
            ...
            log.d { "TripPrice: '${state.tripPrice}' -> ${state.tripPrice.toDoubleOrNull()}" }
            log.d { "WeightUnit: '${state.weightUnit}', CargoWeight: '${state.cargoWeight}'" }

            when (val result = tripRepository.updateTripWithRequest(currentTrip.id, request)) {
                is Result.Success -> {
                    val trip = result.data
                    log.d { "Trip updated. tripPrice: ${trip.tripPrice}, weightUnit: ${trip.weightUnit}" }
    ...
    fun exportCostsToPdf() {
        ...
        log.d { "exportCostsToPdf - hasCosts: ${state.hasCosts}, costsCount: ${state.costs.size}" }
    ...
    suspend fun navigateToAddTripCost() {
        val trip = stateManager.currentTripState.trip ?: return
        stateManager.emitEffect(
            Effect.NavigateToAddTripCost(tripId = trip.id, vehicleId = trip.vehicleId?.toString() ?: "")
        )
    }

    suspend fun navigateToAddPayment() {
        val trip = stateManager.currentTripState.trip ?: return
        stateManager.emitEffect(
            Effect.NavigateToAddPayment(tripId = trip.id, vehicleId = trip.vehicleId?.toString() ?: "")
        )
    }
