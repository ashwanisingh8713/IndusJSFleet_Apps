...
class VehicleFinanceViewModel(
    ...
    private val log = Logger.withTag("VehicleFinanceViewModel")
    ...
    private suspend fun loadData() {
        ...
        try {
            ...
            log.e(e) { "Error loading data: ${e.message}" }
            updateState { copy(isLoading = false, error = e.message) }
        }
    }

    private suspend fun loadPurchaseInfoForVehicles(vehicleIds: List<Int>) {
        val purchasesMap = mutableMapOf<Int, com.indusjs.fleet.domain.entity.finance.VehiclePurchase?>()
        var financedCount = 0
        var cashCount = 0
        var noInfoCount = 0
        var totalLoan = 0.0
    ...
    private suspend fun loadEmiAlerts() {
        when (val result = financeRepository.getEmiAlerts()) {
            ...
            is Result.Error -> {
                log.w { "Failed to load EMI alerts: ${result.message}" }
            }
            is Result.Loading -> { /* Skip */ }
        }
    }
    ...
    private suspend fun selectVehicle(vehicleId: Int) {
        ...
        if (currentState.selectedVehicleId == vehicleId && !currentState.isLoading) {
            log.d { "selectVehicle: Vehicle $vehicleId already selected, skipping API call" }
            return
        }
    ...
    private suspend fun loadLoanDetails(vehicleId: Int) {
        ...
        when (val summaryResult = financeRepository.getLoanSummary(vehicleId)) {
            ...
            is Result.Error -> {
                log.w { "Failed to load loan summary: ${summaryResult.message}" }
            }
            is Result.Loading -> { /* Skip */ }
        }
        ...
        when (val paymentsResult = financeRepository.getLoanPayments(vehicleId, page = 1, perPage = 100)) {
            ...
            is Result.Error -> {
                log.w { "Failed to load loan payments: ${paymentsResult.message}" }
            }
            is Result.Loading -> { /* Skip */ }
        }
    }
