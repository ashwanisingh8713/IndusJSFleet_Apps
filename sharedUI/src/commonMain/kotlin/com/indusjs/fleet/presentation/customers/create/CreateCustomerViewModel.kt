...
class CreateCustomerViewModel(
    ...
    /**
     * Validates primary contact - 10 digits, starts with 6-9.
     */
    ...
    /**
     * Validates secondary contact - optional, but if provided must be valid 10 digits.
     */
    ...
    /**
     * Validates GST number - optional, but if provided must match Indian GST format.
     * Format: 22AAAAA0000A1Z5 (2 state code + 10 PAN + 1 entity + Z + 1 check digit)
     */
    ...
    private suspend fun createCustomer() {
        ...
        when (val result = createCustomerUseCase(
            ...
            is Result.Success -> {
                ...
                sendEffect(Effect.NavigateTo(FleetRoute.CustomerDetail(result.data.id)))
            }
            is Result.Error -> {
                updateState {
                    copy(
