...
/**
 * Composable handler for Fleet Profit & Loss PDF export.
 *
 * @param pdfData The fleet P&L data to export (null when not exporting)
 ...
 */
@Composable
fun FleetProfitLossPdfHandler(
    pdfData: FleetProfitLossPdfData?,
    ...
    LaunchedEffect(pdfData) {
        if (pdfData != null && !isExporting) {
            isExporting = true
            try {
                val result = PdfReportFacade.generateFleetProfitLossReport(pdfData)
                exportResult = result
                if (result.success) {
                    showDialog = true
                } else {
