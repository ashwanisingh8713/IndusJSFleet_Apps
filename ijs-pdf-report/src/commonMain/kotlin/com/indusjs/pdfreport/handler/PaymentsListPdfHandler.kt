...
/**
 * Composable handler for Payments List PDF export.
 *
 * @param pdfData The payments list data to export (null when not exporting)
...
fun PaymentsListPdfHandler(
    ...
    LaunchedEffect(pdfData) {
        if (pdfData != null && !isExporting) {
            isExporting = true
            try {
                val result = PdfReportFacade.generatePaymentsListReport(pdfData)
                exportResult = result
                if (result.success) {
                    showDialog = true
                } else {
