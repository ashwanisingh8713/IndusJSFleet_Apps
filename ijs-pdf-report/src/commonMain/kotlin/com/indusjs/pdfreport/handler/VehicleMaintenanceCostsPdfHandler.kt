...
/**
 * Composable handler for Vehicle Maintenance Costs PDF export.
 * Automatically generates and shows dialog when pdfData is provided.
 *
 * @param pdfData The vehicle maintenance costs data to export (null when not exporting)
 ...
 */
@Composable
fun VehicleMaintenanceCostsPdfHandler(
    pdfData: VehicleMaintenanceCostsPdfData?,
    ...
    LaunchedEffect(pdfData) {
        if (pdfData != null && !isExporting) {
            isExporting = true
            try {
                val result = PdfReportFacade.generateVehicleMaintenanceCostsReport(pdfData)
                exportResult = result
                if (result.success) {
                    showDialog = true
                } else {
