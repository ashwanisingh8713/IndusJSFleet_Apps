...
/**
 * Composable handler for Customer Trips PDF export.
 *
 * @param pdfData The customer trips data to export (null when not exporting)
...
fun CustomerTripsPdfHandler(
    ...
    LaunchedEffect(pdfData) {
        if (pdfData != null && !isExporting) {
            isExporting = true
            try {
                val result = PdfReportFacade.generateCustomerTripsReport(pdfData)
                exportResult = result
                if (result.success) {
                    showDialog = true
                } else {
