...
import com.indusjs.pdfreport.model.VehicleProfitLossPdfData
import com.indusjs.pdfreport.model.PdfExportResult
import com.indusjs.pdfreport.ui.PdfExportDialog
import kotlinx.coroutines.launch

/**
 * Composable handler for Vehicle Profit & Loss PDF export.
 *
 * @param pdfData The vehicle P&L data to export (null when not exporting)
 ...
 */
@Composable
fun VehicleProfitLossPdfHandler(
    pdfData: VehicleProfitLossPdfData?,
    ...
    LaunchedEffect(pdfData) {
        if (pdfData != null && !isExporting) {
            isExporting = true
            try {
                val result = PdfReportFacade.generateVehicleProfitLossReport(pdfData)
                exportResult = result
                if (result.success) {
                    showDialog = true
                } else {
