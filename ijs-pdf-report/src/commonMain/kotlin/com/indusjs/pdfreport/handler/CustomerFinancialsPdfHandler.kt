...
/**
 * Composable handler for Customer Financials PDF export.
 *
 * @param pdfData The customer financials data to export (null when not exporting)
 ...
 */
@Composable
fun CustomerFinancialsPdfHandler(
    pdfData: CustomerFinancialsPdfData?,
    ...
    LaunchedEffect(pdfData) {
        if (pdfData != null && !isExporting) {
            isExporting = true
            try {
                val result = PdfReportFacade.generateCustomerFinancialsReport(pdfData)
                exportResult = result
                if (result.success) {
                    showDialog = true
                } else {
