# AGENTS.md - ijs-driver-lib

## Purpose

**ijs-driver-lib** is a Kotlin Multiplatform library module that encapsulates the **Driver feature's data layer** for the IndusJS Fleet application.
...
```
ijs-driver-lib → ijs-network-lib → ijs-core-lib → ijs-error-lib
                                                 → ijs-dispatcher-lib
                                                 → ijs-datetime-utils
```

> **Note:** `ijs-network-lib` does NOT depend on `ijs-driver-lib` (no circular dependency).
