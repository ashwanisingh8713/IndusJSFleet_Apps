# AGENTS.md - feat-driver

## Purpose

**feat-driver** is a Kotlin Multiplatform library module that encapsulates the **Driver feature's data layer** for the IndusJS Fleet application.
...
```
feat-driver → ijs-network-lib → ijs-core-lib → ijs-error-lib
                                                 → ijs-dispatcher-lib
                                                 → ijs-datetime-utils
```

> **Note:** `ijs-network-lib` does NOT depend on `feat-driver` (no circular dependency).
