...
class VehiclePLViewModel(
    ...
    private suspend fun loadFleetOverview() {
        logger.d(TAG_VEHICLE_PL_VM, "loadFleetOverview: Starting - vehicles=${currentState.vehicles.size}, isLoadingVehicles=${currentState.isLoadingVehicles}")
        ...
        if (currentState.vehicles.isEmpty() && !currentState.isLoadingVehicles) {
            logger.d(TAG_VEHICLE_PL_VM, "loadFleetOverview: No vehicles, returning early")
            updateState {
                copy(
                    isLoading = false,
                    isFleetOverviewMode = true,
        ...
        if (currentState.isLoadingVehicles) {
            logger.d(TAG_VEHICLE_PL_VM, "loadFleetOverview: Still loading vehicles, returning")
            return
        }
        ...
        logger.d(TAG_VEHICLE_PL_VM, "loadFleetOverview: vehicleIdsToUse=$vehicleIdsToUse")

        // Don't make API call with empty vehicle IDs - show empty state instead
        if (vehicleIdsToUse.isEmpty()) {
            logger.d(TAG_VEHICLE_PL_VM, "loadFleetOverview: Empty vehicle IDs, showing empty state")
            updateState {
                copy(
                    isLoading = false,
                    error = null,
        ...
        logger.d(TAG_VEHICLE_PL_VM, "loadFleetOverview: Date range - startDate=$startDate, endDate=$endDate, period=${currentState.period}")
        ...
        logger.d(TAG_VEHICLE_PL_VM, "loadFleetOverview: Calling getMultiVehiclePL with ${request.vehicleIds}")
        ...
        vehicleRepository.getVehicles().collectLatest { result ->
            when (result) {
                is Result.Success<*> -> {
                    ...
                    logger.d(TAG_VEHICLE_PL_VM, "loadVehicles: Success - ${vehicles.size} vehicles loaded")
                    updateState { copy(isLoadingVehicles = false, vehicles = vehicles) }

                    // Log current state for debugging
                    logger.d(TAG_VEHICLE_PL_VM, "loadVehicles: After update - isFleetOverviewMode=${currentState.isFleetOverviewMode}, initialLoadComplete=${currentState.initialLoadComplete}, selectedVehicleId=${currentState.selectedVehicleId}")

                    // Auto-load data after vehicles are loaded
                    if (!currentState.initialLoadComplete) {
                        if (currentState.isFleetOverviewMode) {
                            logger.d(TAG_VEHICLE_PL_VM, "loadVehicles: Calling loadFleetOverview")
                            loadFleetOverview()
                        } else if (currentState.selectedVehicleId != null) {
                            logger.d(TAG_VEHICLE_PL_VM, "loadVehicles: Calling generateReport for ${currentState.selectedVehicleId}")
                            generateReport()
                        } else {
                            logger.d(TAG_VEHICLE_PL_VM, "loadVehicles: No specific mode, marking complete")
                            // No specific mode, just mark as complete
                            updateState { copy(initialLoadComplete = true) }
                        }
                    } else {
                        logger.d(TAG_VEHICLE_PL_VM, "loadVehicles: initialLoadComplete is already true, skipping auto-load")
                    }
                }
                is Result.Error -> {
